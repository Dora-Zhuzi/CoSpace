...
from app.services.generator import generate_document, _write_section, _rewrite_style
...
async def list_available_folders(
    ...
    for folder in folders_result.scalars().all():
        ...
        # 空素材库可直接建项目；有素材时要求全部已完成切分(indexed)
        if not materials or all(m.status == "indexed" for m in materials):
...
# 逐节点生成：根据某个结构树节点的标题 + 挂载片段，生成该节点的一段正文
@router.post("/{project_id}/generate-section")
async def generate_section(
    project_id: str,
    body: SectionGenerate,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    project = await _get_owned_project(db, project_id, current_user)
    ...
    if body.chunk_ids:
        rows = await db.execute(
            ...
            .join(Material, Material.id == MaterialChunk.material_id)
            .where(
                MaterialChunk.id.in_(body.chunk_ids),
                Material.folder_id == project.folder_id,
            )
        )
        by_id = {r[0]: {"content": r[1], "file_name": r[2]} for r in rows.all()}
        # 保持节点挂载片段的原有顺序
        chunks = [by_id[cid] for cid in body.chunk_ids if cid in by_id]
    content = await asyncio.to_thread(_write_section, body.label or "", chunks)
    return {"content": content}


# 文章风格改写：按参考范文风格重写整篇（保留事实/结构）
@router.post("/{project_id}/rewrite-style")
async def rewrite_style(
    ...
    content = await asyncio.to_thread(_rewrite_style, body.content, body.reference, body.strength)
    return {"content": content}
