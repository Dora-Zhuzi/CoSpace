...
CoSpace 不是「输入主题、一键出稿」的黑盒生成器，而是一个**人与 AI 共同创作（AI Co-Creation）**的写作空间：AI 负责发散、检索、起草，**人始终在回路中（Human in the Loop）**——在每一个环节里判断、取舍、修改、定稿。围绕你自己的素材库，与 AI 讨论并沉淀观点，逐步生成写作方案、可编辑的文章结构树（并把素材片段精准挂载到各节点），最终成稿。

> 后端可在**不配置任何大模型 API Key** 的情况下运行：自动降级到本地实现（抽取式摘要 + 拼接式写作 + 哈希向量），开箱即跑、便于体验与开发。

---

## 🤝 设计理念：AI 共创 + Human in the Loop

CoSpace 把写作拆成多个可干预的环节，**每一步的产物都对人开放、可改可删**，而不是让 AI 一口气替你决定一切：

| 环节 | AI 做什么 | 人的控制点（Human in the Loop） |
| --- | --- | --- |
| 共创讨论 | 结合主题与素材抛角度、追问、启发思路 | 主导讨论方向，把有价值的观点/案例/方案手动沉淀为便签 |
| 写作方案 | 依据你筛选的「方案」便签凝练成文 | 方案可多份对比、自由编辑、删除后重来 |
| 结构树 | 生成大纲，并自动筛选、挂载素材片段 | 自由增删章节、手动调整每个节点挂载的素材 |
| 文章生成 | 按结构逐节撰写，并做轻度衔接润色 | 成稿可编辑、下载、保留多版本 |

> 核心原则：**AI 提效，人定调**。所有中间产物（方案 / 结构树 / 文章）都可命名、编辑、删除、留存多份，决策权始终在创作者手里。

---

## ✨ 功能特性

- **素材库**：上传文档，自动解析、切块（按段落）、摘要、向量化入库。
- **AI 共创讨论**：基于主题 + 素材库摘要展开多轮对话；讨论中由你把「观点 / 案例 / 方案」三类思路沉淀为便签，并可把观点/案例反哺回素材库循环利用。
- **写作方案**：依据你确认的「方案」便签一键生成写作方案，可多份、可编辑、可删除。
- **结构树 + 素材挂载**：由方案生成文章结构树，自动用「向量粗筛 + LLM 精挂」把素材片段挂到对应章节；支持手动增删挂载。
- **文章生成**：遍历结构树逐节撰写，末尾再做一次「只加衔接、不改内容」的连贯性润色；生成即保存为一份文章，可编辑、下载、删除。
